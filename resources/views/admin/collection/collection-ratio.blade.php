            ...
            <div class="relative flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400 text-xs font-bold mt-1 uppercase tracking-tight">Input komitmen dan realisasi Collection Ratio</p>
                    </div>
                </div>
                ...
                <div id="content-komitmen" class="space-y-6">
                    <div class="flex items-center justify-between border-b pb-4 mb-6">
                        <div>
                            ...
                            <p class="text-sm text-slate-500 italic">Tentukan target ratio Collection Ratio untuk bulan ini</p>
                        </div>
                    ...
                    @else
                        <form action="{{ route('admin.collection.ratio.store') }}" method="POST" class="max-w-lg mx-auto bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                            @csrf
                            <input type="hidden" name="form_type" value="komitmen">
                            
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Segment</label>
                                <select name="segment" required class="block w-full p-3 text-sm border-2 border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-red-500 focus:border-red-500">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="relative mt-1">
                                    <input type="number" name="target_ratio" min="0" max="100" step="0.01" value="98" required 
                    ...
                    @else
                        <div class="flex items-center justify-between border-b pb-4 mb-6">
                            <div>
                                ...
                                <p class="text-sm text-slate-500 italic">Input pencapaian ratio Collection Ratio hari ini</p>
                            </div>
                        ...
                        <form action="{{ route('admin.collection.ratio.store') }}" method="POST" class="max-w-lg mx-auto bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Segment</label>
                                <select name="segment" required class="block w-full p-3 text-sm border-2 border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-green-500 focus:border-green-500">
                                    <option value="">— Pilih Segment —</option>
                                    <option value="GOV">GOV</option>
                                    <option value="PRIVATE">PRIVATE</option>
                                    <option value="SME">SME</option>
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide text-center">Ratio Aktual (%)</label>
                                <div class="relative mt-1">
                                    <input type="number" name="ratio_aktual" min="0" max="100" step="0.01" required 
                                ...
                                <p class="text-xs text-center text-gray-500 mt-2 italic">Masukkan realisasi ratio aktual hari ini</p>
                            </div>
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Keterangan (Opsional)</label>
                                <input type="text" name="keterangan" 
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <h3 class="text-base font-black text-slate-900 uppercase tracking-wide">📈 Data Collection Ratio Saya</h3>
                </div>
                ...
                <table class="w-full">
                    <thead class="bg-slate-50 border-b border-slate-100">
                        <tr>
                            ...
                            <th class="text-left px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.15em]">Segment</th>
                            <th class="text-right px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.15em]">Target Ratio (%)</th>
                            <th class="text-right px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.15em]">Ratio Aktual (%)</th>
                            ...
                            <td class="px-6 py-4">
                                @php
                                    ...
                                    $segStyle = match($seg) {
                                        ...
                                        'SME'     => 'bg-yellow-50 text-yellow-700',
                                        'SOE'     => 'bg-purple-50 text-purple-700',
                                        default   => 'bg-slate-100 text-slate-600',
                                    };
                                @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded text-[10px] font-black uppercase {{ $segStyle }}">{{ $seg ?: '—' }}</span>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-semibold text-slate-700">{{ $item->target_ratio ? number_format($item->target_ratio, 2).'%' : '—' }}</td>
            ...
            <div class="flex flex-col items-center justify-center py-16 px-8">
                ...
                <p class="text-sm font-bold text-slate-400 uppercase tracking-wider">Belum ada data Collection Ratio</p>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-7xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div class="px-8 py-6 border-b border-slate-200 bg-gradient-to-r from-red-50 to-white flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="text-2xl font-black text-slate-900 tracking-tight">Progress Data Collection Ratio - Semua User</h2>
            ...
            <div class="flex-1 overflow-y-auto p-8">
                    ...
                    <table class="w-full border border-slate-200 rounded-lg">
                        <thead class="bg-slate-800 text-white">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-black uppercase tracking-wider">Segment</th>
                                <th class="px-4 py-3 text-right text-xs font-black uppercase tracking-wider">Target Ratio (%)</th>
                                <th class="px-4 py-3 text-right text-xs font-black uppercase tracking-wider">Ratio Aktual (%)</th>
                                ...
                                <td class="px-4 py-3">
                                    @php
                                        ...
                                        $segColor = match($seg) {
                                            ...
                                            'SME'     => 'bg-yellow-100 text-yellow-700',
                                            'SOE'     => 'bg-purple-100 text-purple-700',
                                            default   => 'bg-slate-100 text-slate-600',
                                        };
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black uppercase {{ $segColor }}">{{ $seg ?: '—' }}</span>
                                </td>
                                <td class="px-4 py-3 text-right text-xs font-semibold text-slate-700">{{ $item->target_ratio ? number_format($item->target_ratio, 2).'%' : '—' }}</td>
                                <td class="px-4 py-3 text-right text-xs font-black text-slate-800">{{ $item->ratio_aktual ? number_format($item->ratio_aktual, 2).'%' : '—' }}</td>
                ...
                <div class="flex flex-col items-center justify-center py-16">
                    ...
                    <p class="text-sm text-slate-400 mt-1">Menunggu user mulai menginput data Collection Ratio</p>
                </div>
                @endif
            </div>
