    ...
    <div class="max-w-7xl mx-auto px-8 py-10">
            ...
            <div class="relative flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400 text-xs font-bold mt-1 uppercase tracking-tight">Kelola data UTIP New & Corrective semua user collection</p>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
            <div class="flex items-center space-x-3 mb-6">
                ...
                <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Input Data UTIP</h2>
            </div>

            <form action="{{ route('admin.collection.utip.store') }}" method="POST">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5 mb-6">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Value</label>
                        <input type="number" name="value" required step="0.01" placeholder="cth: 1000000"
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-8 py-5 border-b border-slate-100">
                <div class="flex items-center space-x-3 mb-4">
                    <div class="w-1 h-6 bg-red-600 rounded-full"></div>
                    <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Data UTIP</h2>
                </div>
                <div class="grid grid-cols-7 gap-3">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Tipe UTIP</label>
                        <select id="filterTipeUtip" onchange="filterTable()"
                            class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 focus:outline-none focus:border-red-400 bg-white">
                ...
                <table class="min-w-full" id="dataTable">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-6 py-3 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Tipe UTIP</th>
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold rounded-md px-2.5 py-1
                                    {{ str_contains($item->type ?? '', 'New') ? 'text-red-700 bg-red-50 border border-red-200' : 'text-slate-700 bg-slate-100 border border-slate-200' }}">
                                    {{ $item->type ?? '—' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $katStyle = match(strtolower($item->category ?? '')) {
                                        'plan'      => 'text-slate-600 bg-slate-100 border border-slate-200',
                                        'komitmen'  => 'text-blue-700 bg-blue-50 border border-blue-200',
                                        'realisasi' => 'text-green-700 bg-green-50 border border-green-200',
                                        default     => 'text-slate-600 bg-slate-100 border border-slate-200',
                                    };
                                @endphp
                                <span class="text-xs font-bold rounded-md px-2.5 py-1 {{ $katStyle }}">
...
function filterTable() {
    ...
    document.querySelectorAll('#dataTable tbody tr').forEach(r => {
        ...
        r.style.display = (
            ...
            (!tu || (c[2].textContent || '').toLowerCase().includes(tu)) &&
            (!k  || (c[3].textContent || '').toLowerCase().includes(k))  &&
...
function resetFilters() {
    ['filterUser', 'filterTipeUtip', 'filterKategori', 'filterBulan', 'filterTahun'].forEach(id => {
