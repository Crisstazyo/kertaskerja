    ...
    <div class="max-w-7xl mx-auto px-8 py-10">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-8">
            <div class="flex border-b border-slate-100">
                ...
                <button onclick="switchTab('komitmen-realisasi')" id="tab-komitmen-realisasi"
                    ...
                    <span>Form Komitmen & Realisasi (Harian)</span>
                </button>
            </div>
                ...
                <div id="content-plan" class="space-y-6">
                    <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Input Plan Bulanan</h2>
                                <p class="text-xs text-slate-400 font-semibold mt-0.5">Tentukan target value UTIP untuk bulan ini.</p>
                            </div>
                        </div>
                    ...
                    @else
                        <form action="{{ route('admin.collection.utip.store') }}" method="POST" class="max-w-md mx-auto space-y-5">
                            @csrf
                            <input type="hidden" name="category" value="plan">
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Tipe UTIP</label>
                                <select name="utip_type" required
                                    class="w-full px-4 py-3 text-sm font-bold text-slate-800 border-2 border-slate-200 rounded-xl focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-100 bg-white transition-colors">
                                    <option value="New UTIP">New UTIP</option>
                                    <option value="Corrective UTIP">Corrective UTIP</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 text-center">Target Value UTIP</label>
                                <input type="number" name="value" min="0" step="0.01" required
                                    ...
                                    placeholder="1000000">
                                <p class="text-xs text-center text-slate-400 font-semibold mt-2">Masukkan target value untuk periode ini</p>
                            </div>
                ...
                <div id="content-komitmen-realisasi" class="hidden space-y-6">
                    @if(!$hasMonthlyCommitment)
                        <div class="flex items-start space-x-4 bg-red-50 border border-red-200 rounded-xl px-5 py-4">
                            ...
                            <div>
                                <p class="text-sm font-black text-red-800">Input Komitmen/Realisasi Belum Tersedia</p>
                    ...
                    @else
                        <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                            <div class="flex items-center space-x-3">
                                <div class="w-1.5 h-8 bg-red-600 rounded-full"></div>
                                <div>
                                    <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Input Komitmen & Realisasi Harian</h2>
                                    <p class="text-xs text-slate-400 font-semibold mt-0.5">Input pencapaian value UTIP hari ini.</p>
                                </div>
                            </div>
                        ...
                        <form action="{{ route('admin.collection.utip.store') }}" method="POST" class="max-w-md mx-auto space-y-5">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 text-center">Value UTIP</label>
                                <input type="number" name="value" min="0" step="0.01" required
                                    ...
                                    placeholder="500000">
                                <p class="text-xs text-center text-slate-400 font-semibold mt-2">Masukkan value komitmen atau realisasi hari ini</p>
                            </div>
                            <div class="flex justify-center">
                                <button type="submit"
                                    class="flex items-center space-x-2 bg-slate-900 hover:bg-red-600 text-white font-bold text-xs px-6 py-3 rounded-xl transition-all duration-200 uppercase tracking-wider shadow-md hover:shadow-lg hover:shadow-red-200">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-6 py-3 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Value</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold rounded-md px-2.5 py-1 {{ str_contains($item->utip_type ?? '', 'New') ? 'text-red-700 bg-red-50 border border-red-200' : 'text-slate-600 bg-slate-50 border border-slate-200' }}">
                                    {{ $item->utip_type ?? '—' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                @if($item->category === 'plan')
                                    <span class="text-xs font-bold rounded-md px-2.5 py-1 text-slate-600 bg-slate-50 border border-slate-200">Plan</span>
            ...
            <div class="py-16 text-center">
                ...
                <p class="text-xs text-slate-300 mt-1">Mulai dengan menginput plan bulanan</p>
            </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-7xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-slate-200">
            ...
            <div class="px-8 py-6 border-b border-slate-100 relative overflow-hidden flex items-center justify-between">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Progress Data UTIP — Semua User</h2>
            ...
            <div class="flex-1 overflow-y-auto p-8">
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                ...
                                <th class="px-4 py-3 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Kategori</th>
                                <th class="px-4 py-3 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Value</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <span class="text-xs font-bold rounded-md px-2 py-0.5 {{ str_contains($item->utip_type ?? '', 'New') ? 'text-red-700 bg-red-50 border border-red-200' : 'text-slate-600 bg-slate-50 border border-slate-200' }}">
                                        {{ $item->utip_type ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    @if($item->category === 'plan')
                                        <span class="text-xs font-bold rounded-md px-2 py-0.5 text-slate-600 bg-slate-50 border border-slate-200">P</span>
                                    @elseif($item->category === 'komitmen')
                ...
                <div class="py-16 text-center">
                    ...
                    <p class="text-xs text-slate-300 mt-1">Menunggu user mulai menginput data UTIP</p>
                </div>
                @endif
            </div>
...
function switchTab(tabName) {
    ...
    document.getElementById('tab-plan').classList.remove('text-red-600', 'border-red-600');
    document.getElementById('tab-plan').classList.add('text-slate-400', 'border-transparent');
    document.getElementById('tab-komitmen-realisasi').classList.remove('text-red-600', 'border-red-600');
    document.getElementById('tab-komitmen-realisasi').classList.add('text-slate-400', 'border-transparent');

    if (tabName === 'plan') {
        document.getElementById('content-plan').classList.remove('hidden');
        document.getElementById('tab-plan').classList.remove('text-slate-400', 'border-transparent');
        document.getElementById('tab-plan').classList.add('text-red-600', 'border-red-600');
        ...
        if (hasCommitment) {
            ...
            document.getElementById('tab-komitmen-realisasi').classList.remove('text-slate-400', 'border-transparent');
            document.getElementById('tab-komitmen-realisasi').classList.add('text-red-600', 'border-red-600');
        }
    }
}
