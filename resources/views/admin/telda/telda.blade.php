        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
            ...
            <form action="{{ route('admin.telda.store') }}" method="POST" id="teldaForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-6">
                    ...
                    @php
                        // Ambil record existing untuk region ini di periode sekarang
                    ...
                    <div class="border border-slate-200 rounded-xl p-5 hover:border-red-200 hover:shadow-sm transition-all duration-200">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            @if($record)
                            <span class="text-[9px] font-black text-blue-600 bg-blue-50 border border-blue-100 rounded px-1.5 py-0.5 uppercase">Update</span>
                            @endif
                        </div>

                        {{-- Hidden: region & status --}}
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number"
                                name="regions[{{ $regionKey }}][commitment]"
                                id="{{ $regionKey }}_commitment"
                                value="{{ $commitmentVal ?? '' }}"
                                placeholder="Masukkan target"
                                min="0"
                                data-telda="{{ $regionKey }}"
                        ...
                        <div>
                            ...
                            <input type="number"
                                name="regions[{{ $regionKey }}][real_ratio]"
                                id="{{ $regionKey }}_real"
                                value="{{ $realVal ?? '' }}"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        {{-- Group history by periode --}}
                        @foreach($historyByPeriode as $periode => $rows)
                        @php
                            // Index rows by region for easy access
