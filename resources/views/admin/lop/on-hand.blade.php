...
@section('title', 'LOP On Hand - Admin')
...
<div class="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 mb-1">📊 LOP On Hand</h1>
                        <p class="text-gray-600">{{ ucfirst($entity) }} - Import & View Data</p>
                    </div>
                </div>
                <a href="{{ route('admin.lop.on_hand.history', $entity) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md">
                    📜 History
                </a>
            </div>
        ...
        <!-- Filter Section -->
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6 border-2 border-green-100">
            <h2 class="text-xl font-bold text-gray-800 mb-4">🔍 Filter Data</h2>
            <form method="GET" action="{{ route('admin.lop.on_hand', $entity) }}">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Bulan</label>
                        <select name="month" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-green-500 focus:border-green-500 p-2.5">
                    ...
                    <div>
                        ...
                        <select name="year" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-green-500 focus:border-green-500 p-2.5">
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-8 py-2.5 rounded-lg font-semibold transition-all duration-300 shadow-md w-full">
                            Filter
                        </button>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6 border-2 border-green-100">
            ...
            <form action="{{ route('admin.lop.upload', [$entity, 'on_hand']) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Bulan</label>
                        <select name="month" required class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-green-500 focus:border-green-500 p-2.5">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Tahun</label>
                        <select name="year" required class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-green-500 focus:border-green-500 p-2.5">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-8 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md whitespace-nowrap">
                                Upload
                            </button>
                        </div>
                        @error('file')
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden border-2 border-green-100">
            <div class="bg-gradient-to-r from-green-500 to-green-600 p-4 text-white">
                <h2 class="text-xl font-bold">📋 Data Terbaru</h2>
                <p class="text-sm text-green-100">Uploaded: {{ $latestImport->created_at->format('d M Y H:i') }} - {{ $latestImport->file_name }}</p>
            </div>
