    ...
    <div class="max-w-7xl mx-auto px-8 py-10">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <h1 class="text-2xl font-black tracking-tight text-slate-900 leading-none uppercase">
                            Rising Star <span class="text-red-600">— {{ $featureTitle }}</span>
                        </h1>
                        <p class="text-slate-400 text-xs font-bold mt-1 uppercase tracking-tight">Kelola data semua user untuk {{ $featureTitle }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
                    <a href="{{ route('admin.rising-star.dashboard', ['star' => $starNumber]) }}"
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
            <div class="flex items-center space-x-3 mb-6">
                ...
                <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Input Data — {{ $featureTitle }}</h2>
            </div>

            <form action="{{ route('admin.rising-star.feature.store', $feature) }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm font-semibold text-slate-800 bg-gray-100" value="Komitmen" disabled>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Target Ratio (%)</label>
                        <input type="number" name="target_ratio" step="0.01" placeholder="110"
            ...
            <div class="px-8 py-5 border-b border-slate-100">
                <div class="flex items-center space-x-3 mb-4">
                    <div class="w-1 h-6 bg-red-600 rounded-full"></div>
                    <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Data {{ $featureTitle }}</h2>
                </div>
            </div>

            {{-- Table --}}
            <div class="overflow-x-auto">
                <table class="min-w-full whitespace-nowrap" id="dataTable">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-4 py-3 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Target Ratio</th>
                            <th class="px-4 py-3 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Bulan</th>
                            <th class="px-4 py-3 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Tahun</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-4 py-3 text-sm font-bold text-slate-400">{{ $index + 1 }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <span class="text-xs font-bold rounded-md px-2.5 py-1 text-red-700 bg-red-50 border border-red-100">Komitmen</span>
                            </td>
                            <td class="px-4 py-3 text-right text-sm font-black text-slate-800">
                                {{ $item->target_ratio ? number_format($item->target_ratio, 2) . '%' : '-' }}
                            </td>
                            <td class="px-4 py-3 text-center text-sm text-slate-600">{{ date('F', mktime(0,0,0,$item->month,1)) }}</td>
                            <td class="px-4 py-3 text-center text-sm text-slate-600">{{ $item->year }}</td>
                        ...
                        <tr>
                            <td colspan="7" class="py-16 text-center">
                                ...
                                <p class="text-sm font-bold text-slate-400">Belum ada data untuk {{ $featureTitle }}</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
