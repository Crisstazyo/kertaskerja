                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <h1 class="text-2xl font-black tracking-tight text-slate-900 leading-none uppercase">
                            Scalling <span class="text-red-600">Telda</span>
                        </h1>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-8">
            <div class="px-8 py-5 border-b border-slate-100 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Input Realisasi — {{ \Carbon\Carbon::now()->format('F Y') }}</h2>
                        <p class="text-xs text-slate-400 font-semibold mt-0.5">Masukkan realisasi untuk masing-masing wilayah Telda.</p>
                    </div>
                </div>
                <span class="text-[10px] font-black tracking-widest text-red-600 bg-red-50 border border-red-100 rounded-md px-3 py-1 uppercase">TELDA</span>
            </div>
            ...
            <div class="p-8">
                ...
                <div class="flex items-start space-x-4 bg-amber-50 border border-amber-200 rounded-xl px-5 py-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-amber-700 mt-0.5">Silakan hubungi admin untuk mengatur commitment bulan ini terlebih dahulu.</p>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-8">
                        ...
                        @php
                            ...
                            $pct           = ($hasCommitment && !is_null($real) && $commitment > 0)
                                             ? ($real / $commitment) * 100 : null;
                        @endphp
                        <div class="border-2 rounded-xl p-5 transition-all duration-200
                            {{ $hasCommitment ? 'border-slate-100 hover:border-red-200 hover:shadow-md hover:shadow-red-500/5' : 'border-slate-100 bg-slate-50 opacity-60' }}">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center space-x-2">
                                    <div class="w-2 h-2 rounded-full {{ $hasCommitment ? 'bg-red-500' : 'bg-slate-300' }}"></div>
                                    <h3 class="text-sm font-black text-slate-800 uppercase tracking-wide">{{ $label }}</h3>
                                </div>
                                @if(!is_null($pct))
                                <span class="text-[10px] font-black rounded-md px-2 py-0.5
                            ...
                            <div class="mb-4">
                                ...
                                @if($hasCommitment)
                                <p class="text-base font-black text-slate-700">{{ number_format($commitment, 0, ',', '.') }}</p>
                                @else
                                <p class="text-sm text-slate-300 font-medium">Belum diatur</p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number"
                                    ...
                                    class="w-full px-3 py-2.5 border-2 border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-100 transition-colors disabled:bg-slate-50 disabled:text-slate-300 disabled:cursor-not-allowed">
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        ...
                        <tr class="bg-slate-50 border-b border-slate-200">
                            ...
                            @foreach($teldas as $key => $label)
                            <th class="px-2 py-2 text-center text-[9px] font-black text-blue-500 uppercase tracking-widest">COM</th>
                            <th class="px-2 py-2 text-center text-[9px] font-black text-green-500 uppercase tracking-widest">REAL</th>
                            @endforeach
                        </tr>
                    </thead>
