        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-6">
                    <a href="{{ route('dashboard.sme') }}" class="text-gray-500 hover:text-gray-700 transition-colors flex items-center gap-2 text-sm font-medium">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // New: if any checkbox to the right of the changed one is already checked,
    // mark all intervening checkboxes before it as checked as well. This allows
    // cascading based purely on column order rather than the stage mapping.
    function cascadeBackwards(checkbox) {
        ...
        // does any later box already have a check?
        ...
        boxes.slice(0, idx).forEach(cb => {
            if (!cb.checked) {
                cb.checked = true;
                // trigger the normal update flow so server gets updated too
    ...
    function updateFunnelCheckbox(checkbox) {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // If checkbox was checked, run both cascades: stage-based and
                // right-to-left based on existing checked boxes.
                if (value) {
    ...
    function updateBillingComplete(checkbox) {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // automatically mirror any sibling checkbox fields
    ...
    function saveCheckboxChange(rowId, dataType, field, value, estNilaiBc, checkboxContainer) {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // auto-toggle siblings if server requests it
                if (data.auto_fields && data.auto_fields.length) {
                    const fields = Array.isArray(data.auto_fields) ? data.auto_fields : [data.auto_fields];
                    const targetVal = data.auto_value === true || data.auto_value === 'true';
