    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">📄UTIP</h1>
                    <p class="text-gray-600 text-lg">Unit Target Income Performance</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8">
            ...
            <div class="p-8">
                <div id="content-komitmen" class="space-y-6">
                    <div class="flex items-center justify-between border-b pb-4">
                        ...
                        <!-- <select id="utip-select" name="type" required onchange="updateCommitment(this)"
                            class="min-w-[220px] px-4 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-100 bg-white">
                            @foreach($utips as $utip => $utip1)
                                <option name="type" value="{{ $utip1->id }}" data-commitment="{{ $utip1->commitment }}">{{ $utip1->type }}</option>
                            @endforeach
                        </select> -->
                    </div>
                        ...
                        <div class="bg-amber-50 border-l-4 border-amber-400 p-6 rounded-r-lg">
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide text-center">Target Collection Ratio</label>
                                <div class="relative mt-1">
                                    <input id="commitment-value" type="text" value="{{ $utips->first()->commitment ?? 0 }}" readonly class="block w-full p-4 text-4xl font-bold text-purple-700 border-2 border-purple-300 rounded-lg bg-purple-50 text-center" placeholder="0.00">
                ...
                <div id="content-realisasi" class="hidden space-y-6">
                    <form action="{{ route('collection.utip.storeRealisasi') }}" method="POST" class="max-w-lg mx-auto space-y-6">
                    <div class="flex items-center justify-between border-b pb-4">
                        <div>
                        ...
                        <select id="utip-select" name="utip_id" required onchange="updateCommitment(this)"
                            class="min-w-[220px] px-4 py-2 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-100 bg-white">
                            @foreach($utips as $utip => $utip1)
                                <option value="{{ $utip1->id }}" data-commitment="{{ $utip1->commitment }}" data-type="{{ $utip1->type }}">{{ $utip1->type }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Realisasi UTIP (%)</label>
                            <div class="relative">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-100 border-b border-gray-200">
                            ...
                            <th class="p-4 font-bold text-xs uppercase text-gray-600 text-center">Segment</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($activities as $activity)
                            <tr class="{{ Str::contains($activity->type, 'UTIP') ? 'bg-purple-50/30' : '' }}">
                                ...
                                <td class="p-4 text-center font-bold {{ Str::contains($activity->type, 'UTIP') ? 'text-purple-700' : 'text-green-600' }}">
                                    {{ $activity->type}}
                                </td>
                                <td class="p-4 text-center font-bold {{ Str::contains($activity->type, 'UTIP') ? 'text-purple-700' : 'text-green-600' }}">
