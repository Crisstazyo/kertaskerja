...
@section('title', 'Profiling Overall HOTD')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-100 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">📊 Profiling Overall HOTD</h1>
                    <p class="text-gray-600 text-lg">Input Komitmen & Realisasi Profiling Overall HOTD</p>
                </div>
                <a href="{{ route('rising-star.rising-star-2') }}" class="bg-teal-500 hover:bg-teal-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md hover:shadow-lg">
                    ← Kembali
                </a>
            </div>
            <div class="h-1 bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-600 rounded-full"></div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
            ...
            <div id="form-komitmen" class="tab-content p-8">
                ...
                @else
                    <form method="POST" action="{{ route('rising-star.profiling-overall-hotd.store') }}">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="relative mt-1 mb-6">
                                <input type="text" value="110" readonly class="block w-full p-4 text-4xl font-bold text-teal-700 border-2 border-teal-300 rounded-lg bg-teal-50 text-center cursor-not-allowed" placeholder="110">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                    <span class="text-teal-500 font-bold text-2xl">%</span>
                                </div>
                            </div>
                            <p class="text-xs text-center text-gray-500 mb-6 italic">Target ratio tetap untuk semua profiling Overall HOTD</p>
                        </div>

                        <button type="submit" class="w-full bg-gradient-to-r from-teal-500 to-cyan-600 text-white font-bold py-4 px-8 rounded-xl hover:from-teal-600 hover:to-cyan-700 transition transform hover:scale-105 shadow-lg">
                            💾 Simpan Komitmen
                        </button>
                    </div>
                </form>
            ...
            <div id="form-realisasi" class="tab-content p-8 hidden">
                <form method="POST" action="{{ route('rising-star.profiling-overall-hotd.store') }}">
                    ...
                    <div class="space-y-6 max-w-2xl mx-auto">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">� Ratio Profiling (Realisasi) (%)</label>
                            <div class="relative">
                                <input type="number" step="0.01" name="ratio_aktual" placeholder="Masukkan ratio (contoh: 105.00)" required
                                       class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-teal-500 focus:ring focus:ring-teal-200 transition">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                    <span class="text-gray-500 font-bold text-xl">%</span>
                                </div>
                            </div>
                            <p class="text-sm text-gray-500 mt-2">Ratio realisasi profiling yang telah dicapai</p>
                        </div>

                        <button type="submit" class="w-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-4 px-8 rounded-xl hover:from-cyan-600 hover:to-blue-700 transition transform hover:scale-105 shadow-lg">
                            ✅ Simpan Realisasi
                        </button>
                    </div>
                </form>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 bg-gradient-to-r from-teal-500 to-cyan-600">
                <h3 class="text-2xl font-bold text-white">📊 Data Profiling Overall HOTD</h3>
                <p class="text-teal-100 mt-1">Menampilkan semua data profiling Overall HOTD</p>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <form method="POST" action="{{ route('rising-star.profiling-overall-hotd.delete', $item->id) }}" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
...
<style>
    ...
    .tab-button.active {
        color: #14B8A6;
        background: linear-gradient(to bottom, transparent 0%, rgba(20, 184, 166, 0.1) 100%);
