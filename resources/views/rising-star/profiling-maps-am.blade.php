...
@section('title', 'Profiling MAPS AM')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-100 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">📊 Profiling MAPS AM</h1>
                    <p class="text-gray-600 text-lg">Input Komitmen & Realisasi Profiling MAPS Area Manager</p>
                </div>
                <a href="{{ route('rising-star.rising-star-2') }}" class="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md hover:shadow-lg">
                    ← Kembali
                </a>
            </div>
            <div class="h-1 bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-600 rounded-full"></div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
            ...
            <div id="form-komitmen" class="tab-content p-8">
                ...
                @else
                    <form method="POST" action="{{ route('rising-star.profiling-maps-am.store') }}">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="relative mt-1 mb-6">
                                <input type="text" value="110" readonly class="block w-full p-4 text-4xl font-bold text-orange-700 border-2 border-orange-300 rounded-lg bg-orange-50 text-center cursor-not-allowed" placeholder="110">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                    <span class="text-orange-500 font-bold text-2xl">%</span>
                                </div>
                            </div>
                            <p class="text-xs text-center text-gray-500 mb-6 italic">Target ratio tetap untuk semua profiling MAPS AM</p>
                        </div>

                        <button type="submit" class="w-full bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold py-4 px-8 rounded-xl hover:from-orange-600 hover:to-amber-700 transition transform hover:scale-105 shadow-lg">
                            💾 Simpan Komitmen
                        </button>
                    </div>
                </form>
            ...
            <div id="form-realisasi" class="tab-content p-8 hidden">
                <form method="POST" action="{{ route('rising-star.profiling-maps-am.store') }}">
                    ...
                    <div class="space-y-6 max-w-2xl mx-auto">
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="ratio_aktual" placeholder="Masukkan ratio (contoh: 105.00)" required
                                       class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-orange-500 focus:ring focus:ring-orange-200 transition">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                        ...
                        <button type="submit" class="w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-white font-bold py-4 px-8 rounded-xl hover:from-amber-600 hover:to-yellow-700 transition transform hover:scale-105 shadow-lg">
                            ✅ Simpan Realisasi
                        </button>
                    </div>
                </form>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 bg-gradient-to-r from-orange-500 to-amber-600">
                <h3 class="text-2xl font-bold text-white">📊 Data Profiling MAPS AM</h3>
                <p class="text-orange-100 mt-1">Menampilkan semua data profiling MAPS Area Manager</p>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <form method="POST" action="{{ route('rising-star.profiling-maps-am.delete', $item->id) }}" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
