...
@section('title', 'Visiting AM')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-100 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">👨‍💼 Visiting AM</h1>
                    <p class="text-gray-600 text-lg">Input Komitmen & Realisasi Visiting Area Manager</p>
                </div>
                <a href="{{ route('rising-star.rising-star-1') }}" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md hover:shadow-lg">
                    ← Kembali
                </a>
            </div>
            <div class="h-1 bg-gradient-to-r from-green-500 via-emerald-500 to-teal-600 rounded-full"></div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
            ...
            <div id="form-komitmen" class="tab-content p-8">
                ...
                @else
                    <form method="POST" action="{{ route('rising-star.visiting-am.store') }}">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="relative mt-1 mb-6">
                                <input type="text" value="110" readonly class="block w-full p-4 text-4xl font-bold text-green-700 border-2 border-green-300 rounded-lg bg-green-50 text-center cursor-not-allowed" placeholder="110">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                    <span class="text-green-500 font-bold text-2xl">%</span>
                                </div>
                            </div>
                            <p class="text-xs text-center text-gray-500 mb-6 italic">Target ratio tetap untuk semua visiting AM</p>
                        </div>

                        <button type="submit" class="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-4 px-8 rounded-xl hover:from-green-600 hover:to-emerald-700 transition transform hover:scale-105 shadow-lg">
                            💾 Simpan Komitmen
                        </button>
                    </div>
                </form>
            ...
            <div id="form-realisasi" class="tab-content p-8 hidden">
                <form method="POST" action="{{ route('rising-star.visiting-am.store') }}">
                    ...
                    <div class="space-y-6 max-w-2xl mx-auto">
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="ratio_aktual" placeholder="Masukkan ratio (contoh: 105.00)" required
                                       class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-green-500 focus:ring focus:ring-green-200 transition">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                        ...
                        <button type="submit" class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold py-4 px-8 rounded-xl hover:from-emerald-600 hover:to-teal-700 transition transform hover:scale-105 shadow-lg">
                            ✅ Simpan Realisasi
                        </button>
                    </div>
                </form>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 bg-gradient-to-r from-green-500 to-emerald-600">
                <h3 class="text-2xl font-bold text-white">📊 Data Visiting AM</h3>
                <p class="text-green-100 mt-1">Menampilkan semua data visiting Area Manager</p>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <form method="POST" action="{{ route('rising-star.visiting-am.delete', $item->id) }}" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
...
<style>
    ...
    .tab-button.active {
        color: #10B981;
        background: linear-gradient(to bottom, transparent 0%, rgba(16, 185, 129, 0.1) 100%);
