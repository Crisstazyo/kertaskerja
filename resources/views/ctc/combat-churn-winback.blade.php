...
@section('title', 'Winback - Combat the Churn')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-green-50 via-teal-50 to-emerald-100 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">🔄 Winback</h1>
                    <p class="text-gray-600 text-lg">Customer Winback Strategy - SSL Tracking</p>
                </div>
                <a href="{{ route('ctc.combat-the-churn') }}" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md hover:shadow-lg">
                    ← Kembali
                </a>
            </div>
            <div class="h-1 bg-gradient-to-r from-green-500 via-teal-500 to-emerald-600 rounded-full"></div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
            ...
            <div class="border-b border-gray-200">
                <nav class="flex -mb-px">
                    <button onclick="showTab('komitmen')" id="tab-komitmen" class="tab-button group relative min-w-0 flex-1 overflow-hidden bg-green-50 py-4 px-4 text-center text-sm font-medium hover:bg-green-100 focus:z-10 border-b-2 border-green-500 text-green-600">
            ...
            <div class="p-8">
                ...
                <div id="content-komitmen" class="tab-content">
                    <div class="flex items-center justify-between border-b pb-4 mb-6">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-800">Input Komitmen Winback</h2>
                            <p class="text-sm text-gray-500 italic">Masukkan target komitmen SSL per region</p>
                        </div>
                        <span class="bg-green-100 text-green-700 text-xs font-semibold px-3 py-1 rounded-full uppercase">Komitmen</span>
                    </div>
                    ...
                    @else
                        <form method="POST" action="{{ route('ctc.combat-the-churn.store') }}" class="max-w-lg mx-auto space-y-6">
                            ...
                            <button type="submit" class="w-full bg-gradient-to-r from-green-500 to-teal-600 text-white py-4 px-6 rounded-lg font-bold text-lg hover:from-green-600 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                                💾 Simpan Komitmen
                            </button>
                        </form>
                    @endif
                ...
                <div id="content-realisasi" class="tab-content hidden">
                    <div class="flex items-center justify-between border-b pb-4 mb-6">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-800">Input Realisasi Winback</h2>
                            <p class="text-sm text-gray-500 italic">Masukkan realisasi SSL per region</p>
                        </div>
                        <span class="bg-emerald-100 text-emerald-700 text-xs font-semibold px-3 py-1 rounded-full uppercase">Realisasi</span>
                    </div>
                    ...
                    <form method="POST" action="{{ route('ctc.combat-the-churn.store') }}" class="max-w-lg mx-auto space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">📊 Kuantitas SSL</label>
                            <input type="number" name="quantity" step="1" min="0" required
                                   class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent" 
                                   placeholder="Masukkan jumlah SSL (contoh: 90)">
                            <p class="mt-1 text-xs text-gray-500">Jumlah SSL dalam satuan kuantitas</p>
                        </div>

                        <button type="submit" class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white py-4 px-6 rounded-lg font-bold text-lg hover:from-emerald-600 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                            💾 Simpan Realisasi
                        </button>
                    </form>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="p-4">
                                <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $item->type == 'komitmen' ? 'bg-green-100 text-green-700' : 'bg-emerald-100 text-emerald-700' }}">
                                    {{ ucfirst($item->type) }}
                                </span>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="p-8 text-center text-gray-500">
                                ...
                                <p class="text-lg font-semibold">Belum ada data Winback</p>
...
function showTab(tabName) {
    ...
    document.querySelectorAll('.tab-button').forEach(button => {
        button.classList.remove('bg-green-50', 'text-green-600', 'border-green-500');
    ...
    activeButton.classList.add('bg-green-50', 'text-green-600', 'border-green-500');
}
</script>
@endsection
