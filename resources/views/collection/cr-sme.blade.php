    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">📊 CR SME</h1>
                    <p class="text-gray-600 text-lg">Collection Ratio - Small Medium Enterprise</p>
                </div>
                <a href="{{ route('collection.collection-ratio') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 shadow-md hover:shadow-lg">
                    ← Kembali
                </a>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8">
            <div class="flex border-b border-gray-100 bg-gray-50/50">
                <button onclick="switchTab('komitmen')" id="tab-komitmen" class="flex-1 py-4 text-center font-bold text-blue-600 border-b-4 border-blue-500 transition-all">
                    📝 Form Komitmen (Bulanan)
                </button>
                <button onclick="switchTab('realisasi')" id="tab-realisasi" class="flex-1 py-4 text-center font-bold {{ $hasMonthlyCommitment ? 'text-gray-500 hover:text-blue-500' : 'text-gray-300 cursor-not-allowed' }} border-b-4 border-transparent transition-all" {{ !$hasMonthlyCommitment ? 'disabled' : '' }}>
            ...
            <div class="p-8">
                <div id="content-komitmen" class="space-y-6">
                    <div class="flex items-center justify-between border-b pb-4">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-800">Input Komitmen Bulanan - SME</h2>
                            <p class="text-sm text-gray-500 italic">Tentukan target collection ratio SME untuk bulan ini.</p>
                        </div>
                        <span class="bg-blue-100 text-blue-700 text-xs font-semibold px-3 py-1 rounded-full uppercase">Periode: {{ now()->translatedFormat('F Y') }}</span>
                    </div>
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-amber-700">
                                        Anda telah menginput target ratio CR SME untuk periode <strong>{{ now()->translatedFormat('F Y') }}</strong>. Input baru hanya tersedia di bulan depan.
                                    </p>
                                </div>
                            </div>
                        </div>
                    @else
                        <form action="{{ route('cr-sme.storeKomitmen') }}" method="POST" class="max-w-md mx-auto bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                            @csrf
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide text-center">Target Ratio (%)</label>
                                <div class="relative mt-1">
                                    <input type="number" step="0.01" name="target_ratio" required class="block w-full p-4 text-2xl font-bold text-blue-700 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="0.00">
                                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                        <span class="text-gray-500 font-bold">%</span>
                                    </div>
                                </div>
                            </div>
                            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-4 rounded-lg font-bold shadow-lg transition-all transform hover:-translate-y-1">
                ...
                <div id="content-realisasi" class="hidden space-y-6">
                    ...
                    @else
                        <div class="flex items-center justify-between border-b pb-4">
                            <div>
                                <h2 class="text-2xl font-bold text-gray-800">Input Realisasi CR SME</h2>
                        ...
                        <form action="{{ route('cr-sme.storeRealisasi') }}" method="POST" class="max-w-lg mx-auto space-y-6">
                            @csrf
                            <div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($data as $item)
                            <tr class="{{ $item->type == 'komitmen' ? 'bg-blue-50/30' : '' }}">
                                <td class="p-4 text-sm text-gray-600 font-medium">{{ $item->entry_date->translatedFormat('d M Y, H:i') }}</td>
                                <td class="p-4">
                                    @if($item->type == 'komitmen')
                                        <span class="bg-blue-600 text-white px-2 py-1 rounded text-[10px] font-bold uppercase">Komitmen</span>
                                    @else
                                        <span class="bg-green-600 text-white px-2 py-1 rounded text-[10px] font-bold uppercase">Realisasi</span>
                                    @endif
                                </td>
                                <td class="p-4 text-right">
                                    <span class="font-bold text-lg {{ $item->type == 'komitmen' ? 'text-blue-700' : 'text-green-700' }}">
